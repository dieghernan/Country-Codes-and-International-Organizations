...
#4 Wikistyle----
library(jsonlite)
ISOfull = fromJSON("outputs/Countrycodesfull.json") %>% as.data.frame()
ISO_orgnames = function(df) {
  orgs_index = unlist(df[1, "orgs"]) %>% as.data.frame(stringsAsFactors =
                                                         F)
...
ISO_memcol = function(df, orgtosearch) {
  ind = match(orgtosearch, unlist(df[1, "orgs"]))
  or = lapply(1:nrow(df), function(x)
    unlist(df[x, "orgs_member"])[ind])
...
svg("outputs/test_AU.svg",width = 550/90,height = 550/90)
#Plotting
par(mar = c(0, 0, 0, 0))
...
#               "files/Orthographic_gradient.svg"
#               )
grad=as.raster(rsvg("files/Orthographic_gradient.svg"))
